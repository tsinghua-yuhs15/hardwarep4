...
{
    volatile unsigned int* temp_working_addr_start = p->transport->mapchannelReq(p, CHAN_NUM_MainRequest_forward_add_entry, 3);
    volatile unsigned int* temp_working_addr = temp_working_addr_start;
    if (p->transport->busywait(p, CHAN_NUM_MainRequest_forward_add_entry, "MainRequest_forward_add_entry")) return 1;
    p->transport->write(p, &temp_working_addr, (key.nhop_ipv4>>30)|(((unsigned long)key.padding)<<2));
    p->transport->write(p, &temp_working_addr, val._action|(((unsigned long)key.nhop_ipv4)<<2));
    p->transport->send(p, temp_working_addr_start, (CHAN_NUM_MainRequest_forward_add_entry << 16) | 3, -1);
    return 0;
};

int MainRequest_ipv4_lpm_add_entry ( struct PortalInternal *p, const Ipv4LpmReqT key, const Ipv4LpmRspT val )
{
    volatile unsigned int* temp_working_addr_start = p->transport->mapchannelReq(p, CHAN_NUM_MainRequest_ipv4_lpm_add_entry, 3);
    volatile unsigned int* temp_working_addr = temp_working_addr_start;
    if (p->transport->busywait(p, CHAN_NUM_MainRequest_ipv4_lpm_add_entry, "MainRequest_ipv4_lpm_add_entry")) return 1;
    p->transport->write(p, &temp_working_addr, (key.dstAddr>>30)|(((unsigned long)key.padding)<<2));
    p->transport->write(p, &temp_working_addr, val._action|(((unsigned long)key.dstAddr)<<2));
    p->transport->send(p, temp_working_addr_start, (CHAN_NUM_MainRequest_ipv4_lpm_add_entry << 16) | 3, -1);
    return 0;
};

int MainRequest_send_frame_add_entry ( struct PortalInternal *p, const SendFrameReqT key, const SendFrameRspT val )
{
    volatile unsigned int* temp_working_addr_start = p->transport->mapchannelReq(p, CHAN_NUM_MainRequest_send_frame_add_entry, 2);
    volatile unsigned int* temp_working_addr = temp_working_addr_start;
    if (p->transport->busywait(p, CHAN_NUM_MainRequest_send_frame_add_entry, "MainRequest_send_frame_add_entry")) return 1;
    p->transport->write(p, &temp_working_addr, val._action|(((unsigned long)key.egress_port)<<2));
    p->transport->send(p, temp_working_addr_start, (CHAN_NUM_MainRequest_send_frame_add_entry << 16) | 2, -1);
    return 0;
};
...
int MainRequest_handleMessage(struct PortalInternal *p, unsigned int channel, int messageFd)
{
    ...
    case CHAN_NUM_MainRequest_forward_add_entry: {
        ...
        p->transport->recv(p, temp_working_addr, 2, &tmpfd);
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.forward_add_entry.key.nhop_ipv4 = (uint32_t)(((uint32_t)(((tmp)&0x3ul))<<30));
        tempdata.forward_add_entry.key.padding = (uint8_t)(((tmp>>2)&0xful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.forward_add_entry.val._action = (uint8_t)(((tmp)&0x3ul));
        tempdata.forward_add_entry.key.nhop_ipv4 |= (uint32_t)(((tmp>>2)&0x3ffffffful));
    ...
    case CHAN_NUM_MainRequest_ipv4_lpm_add_entry: {
        ...
        p->transport->recv(p, temp_working_addr, 2, &tmpfd);
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.ipv4_lpm_add_entry.key.dstAddr = (uint32_t)(((uint32_t)(((tmp)&0x3ul))<<30));
        tempdata.ipv4_lpm_add_entry.key.padding = (uint8_t)(((tmp>>2)&0xful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.ipv4_lpm_add_entry.val._action = (uint8_t)(((tmp)&0x3ul));
        tempdata.ipv4_lpm_add_entry.key.dstAddr |= (uint32_t)(((tmp>>2)&0x3ffffffful));
    ...
    case CHAN_NUM_MainRequest_send_frame_add_entry: {
        ...
        p->transport->recv(p, temp_working_addr, 1, &tmpfd);
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.send_frame_add_entry.val._action = (uint8_t)(((tmp)&0x3ul));
        tempdata.send_frame_add_entry.key.egress_port = (uint16_t)(((tmp>>2)&0x1fful));
