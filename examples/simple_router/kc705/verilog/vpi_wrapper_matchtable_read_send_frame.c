 ...
 *     #include "vpi_wrapper_matchtable_read_send_frame.h"
 *     void (*vlog_startup_routines[])() = { matchtable_read_send_frame_vpi_register, 0u };
 * 
 * For a Verilog simulator which requires a .tab file, use the following entry:
 * $imported_matchtable_read_send_frame call=matchtable_read_send_frame_calltf size=2 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_matchtable_read_send_frame vpiSysFuncSized 2 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
char matchtable_read_send_frame(unsigned int );

/* VPI wrapper function */
PLI_INT32 matchtable_read_send_frame_calltf(PLI_BYTE8 *user_data)
{
  vpiHandle hCall;
  unsigned int arg_1;
  char vpi_result;
  ...
  vpi_result = matchtable_read_send_frame(arg_1);
...
/* sft: $imported_matchtable_read_send_frame vpiSysFuncSized 2 unsigned */

/* tab: $imported_matchtable_read_send_frame call=matchtable_read_send_frame_calltf size=2 acc=rw:%TASK */

PLI_INT32 matchtable_read_send_frame_sizetf(PLI_BYTE8 *user_data)
{
  return 2u;
}

/* VPI wrapper registration function */
void matchtable_read_send_frame_vpi_register()
{
  ...
  tf_data.tfname = "$imported_matchtable_read_send_frame";
  tf_data.calltf = matchtable_read_send_frame_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = matchtable_read_send_frame_sizetf;
  tf_data.user_data = "$imported_matchtable_read_send_frame";
