 ...
 *     #include "vpi_wrapper_matchtable_read_ipv4_lpm.h"
 *     void (*vlog_startup_routines[])() = { matchtable_read_ipv4_lpm_vpi_register, 0u };
 * 
 * For a Verilog simulator which requires a .tab file, use the following entry:
 * $imported_matchtable_read_ipv4_lpm call=matchtable_read_ipv4_lpm_calltf size=2 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_matchtable_read_ipv4_lpm vpiSysFuncSized 2 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
char matchtable_read_ipv4_lpm(unsigned long long );

/* VPI wrapper function */
PLI_INT32 matchtable_read_ipv4_lpm_calltf(PLI_BYTE8 *user_data)
{
  vpiHandle hCall;
  ...
  vpi_result = matchtable_read_ipv4_lpm(arg_1);
...
/* sft: $imported_matchtable_read_ipv4_lpm vpiSysFuncSized 2 unsigned */

/* tab: $imported_matchtable_read_ipv4_lpm call=matchtable_read_ipv4_lpm_calltf size=2 acc=rw:%TASK */

PLI_INT32 matchtable_read_ipv4_lpm_sizetf(PLI_BYTE8 *user_data)
{
  return 2u;
}

/* VPI wrapper registration function */
void matchtable_read_ipv4_lpm_vpi_register()
{
  ...
  tf_data.tfname = "$imported_matchtable_read_ipv4_lpm";
  tf_data.calltf = matchtable_read_ipv4_lpm_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = matchtable_read_ipv4_lpm_sizetf;
  tf_data.user_data = "$imported_matchtable_read_ipv4_lpm";
